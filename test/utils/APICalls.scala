...
object APICalls {
  ...
  def createSession(userController: UserController, userId: String, password: String): String = {
    val sessionIdRes =
      userController.login(userId, password).apply(FakeRequest(GET, s"/users/login/$userId/$password}"))
  ...
  def createAccount(
      ...
      userId1: String,
      userId2: List[String] = List.empty,
      sessionId: String,
  ...
  def getAccount(accountsController: AccountsController, accountId: String): String = {
    val getAccountRes =
      accountsController.getAccountById(accountId).apply(FakeRequest(GET, s"/accounts/getAccount/$accountId"))
