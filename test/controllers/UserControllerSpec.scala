...
import models.{CreateUserRequest, FieldDetails, UpdateUserRequest}
...
class UserControllerSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  "UserController" should {
    ...
    "valid positive cases of Users APIs" in {
      ...
      val sessionIdRes = userController.login(userId, createUserRequest.password).apply(FakeRequest(GET, s"/users/login/$userId/${createUserRequest.password}"))
      ...
      val updatedUserRes = userController.updateUser().apply(FakeRequest(PUT, "/users/update").withBody(updateUserRequestJson)
        .withHeaders(("Content-Type", "application/json")))
