...
class UserController @Inject() (val controllerComponents: ControllerComponents, userDAO: UserDAO)(
  ...
  def createBankUser(): Action[JsValue] = Action.async(parse.json) { implicit request =>
    ...
    userDAO
      .createUser(createUserRequest, userId)
      .map { x =>
        Ok(
          Json.obj(
            "status"        -> 200,
  ...
  def updateUser(): Action[JsValue] = Action.async(parse.json) { implicit request =>
    ...
    userDAO
      .updateUser(updateUserRequest.userId, updateUserRequest.sessionId, updateUserRequest.updates)
      .map { updatedRows =>
        Ok(
          Json.obj(
            "status"            -> 200,
