...
class AccountsDAO @Inject() (configuration: Configuration, crud: CRUD, userDAO: UserDAO)(
  ...
  private def insertToMappingTbl(
    ...
    if (accountTypeId == "1") {
      ...
      val valuesToInsertInMapAccTbl =
        allUsers.map(userId => Seq(accountId, userId).map(value => s"\'$value\'").mkString(","))
  ...
  def getAccountById(id: String): Future[Account] = {
    ...
    for {
      ...
      Account(
        ...
        accountTypeId = accountInfo.head.accountTypeId,
        balance = accountInfo.head.balance
      )
    }
  }
  ...
  def isAccountValid(accountId: String): Future[Boolean] = {
    crud
      .select[Int](
        tableName = accountsTableName,
        columnsToRetrive = Some("count(*)"),
        condition = Some(s"WHERE id=\'$accountId\'")
      )
