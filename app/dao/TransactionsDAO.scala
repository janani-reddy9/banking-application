...
import models.{Account, CreateTransactionRequest, Transaction}
...
class TransactionsDAO @Inject()(@NamedCache("transactions-cache") transactionsCache: AsyncCacheApi, configuration: Configuration, crud: CRUD, userDAO: UserDAO, accountsDAO: AccountsDAO)(implicit ec: ExecutionContext) {
  ...
  def create(transactionId: String, request: CreateTransactionRequest): Future[Int] = {
    ...
    val balance = for {
      ...
      require(request.transactionType == "withdraw" || request.transactionType == "deposit", "Transaction type is not supported")
    ...
    val valuesToInsertInTransTbl = Seq(s"\'$transactionId\'", s"\'${request.accountId}\'", s"\'${request.userId}\'", s"\'${request.transactionType}\'", request.amount, s"\'${System.currentTimeMillis()}\'").mkString(", ")
    balance.flatMap(
      value => {
        ...
        transaction.flatMap(insertedRows => crud.update(accountsDAO.accountsTableName, s"balance = $value", s"id = \'${request.accountId}\'"))
